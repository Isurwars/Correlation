      ...
      [&](size_t i) {
        ...
        for (size_t type1 = 0; type1 < num_elements; ++type1) {
          for (size_t type2 = 0; type2 < num_elements; ++type2) {
            if (!distance_local[type1][type2].empty()) {
              distance_tensor_[type1][type2].insert(
                  distance_tensor_[type1][type2].end(),
                  distance_local[type1][type2].begin(),
                  distance_local[type1][type2].end());
            }
          }
        }
        for (size_t atom_idx = 0; atom_idx < atom_count; ++atom_idx) {
          if (!neighbor_local[atom_idx].empty()) {
            neighbor_tensor_[atom_idx].insert(neighbor_tensor_[atom_idx].end(),
                                              neighbor_local[atom_idx].begin(),
                                              neighbor_local[atom_idx].end());
          }
        }
