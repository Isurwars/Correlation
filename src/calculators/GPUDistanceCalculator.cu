...
__global__ void distance_kernel(
    ...
    const unsigned long long *__restrict__ bin_offsets,
    const unsigned long long *__restrict__ bin_indices,
    GPULattice lattice,
    GPUSearchGrid grid,
    double cutoff_sq,
    const double *__restrict__ bond_cutoffs_sq,
    int num_elements,
    bool ignore_periodic_self_interactions,
    int num_atoms,
    unsigned long long *distance_counter,
    unsigned long long *bond_counter,
    GPUDistance *__restrict__ distances,
    GPUBond *__restrict__ bonds) {
  ...
  double a_x = wrapped_x[i];
  double a_y = wrapped_y[i];
  double a_z = wrapped_z[i];
  int type_A = element_ids[i];

  int c_bin = atom_bin[i];
  ...
  for (int dx = -grid.max_dx; dx <= grid.max_dx; ++dx) {
    int nx_bin = c_x + dx;
    int shift_x = (nx_bin >= 0) ? (nx_bin / grid.K_x) : ((nx_bin - grid.K_x + 1) / grid.K_x);
    int wrap_x = nx_bin - shift_x * grid.K_x;

    for (int dy = -grid.max_dy; dy <= grid.max_dy; ++dy) {
      int ny_bin = c_y + dy;
      int shift_y = (ny_bin >= 0) ? (ny_bin / grid.K_y) : ((ny_bin - grid.K_y + 1) / grid.K_y);
      int wrap_y = ny_bin - shift_y * grid.K_y;

      for (int dz = -grid.max_dz; dz <= grid.max_dz; ++dz) {
        int nz_bin = c_z + dz;
        int shift_z = (nz_bin >= 0) ? (nz_bin / grid.K_z) : ((nz_bin - grid.K_z + 1) / grid.K_z);
        int wrap_z = nz_bin - shift_z * grid.K_z;
        ...
        unsigned long long start = bin_offsets[n_bin_idx];
        unsigned long long end = bin_offsets[n_bin_idx + 1];

        for (unsigned long long offset = start; offset < end; ++offset) {
          int j = static_cast<int>(bin_indices[offset]);
          if (j < i) {
            continue;
          }
          if (i == j && zero_disp) {
          ...
          double shifted_x = wrapped_x[j] + disp_x;
          double shifted_y = wrapped_y[j] + disp_y;
          double shifted_z = wrapped_z[j] + disp_z;

          double dx_val = shifted_x - a_x;
          double dy_val = shifted_y - a_y;
          double dz_val = shifted_z - a_z;
          ...
          if (d_sq <= max_bond_dist_sq) {
            ...
            double precise_disp_x = shifted_x - wrapped_x[j];
            double precise_disp_y = shifted_y - wrapped_y[j];
            double precise_disp_z = shifted_z - wrapped_z[j];

            double r_x = (wrapped_x[j] - a_x) + precise_disp_x;
            double r_y = (wrapped_y[j] - a_y) + precise_disp_y;
            double r_z = (wrapped_z[j] - a_z) + precise_disp_z;
            ...
            if (WriteMode) {
              ...
              bonds[idx] = GPUBond{i, j, precise_dist, r_x, r_y, r_z};
            } else {
...
void compute_distances_gpu(const correlation::core::Cell &cell, double cutoff_sq,
  ...
  std::vector<double> h_bond_cutoffs_sq(num_elements * num_elements, 0.0);
  for (size_t i = 0; i < num_elements; ++i) {
    for (size_t j = 0; j < num_elements; ++j) {
      if (i < bond_cutoffs_sq.size() && j < bond_cutoffs_sq[i].size()) {
        h_bond_cutoffs_sq[i * num_elements + j] = bond_cutoffs_sq[i][j];
      }
    }
  }
  ...
  hipMemcpy(d_bond_cutoffs_sq, h_bond_cutoffs_sq.data(), num_elements * num_elements * sizeof(double), hipMemcpyHostToDevice);
  ...
  hipLaunchKernelGGL(distance_kernel<false>, grid_size, block_size, 0, 0,
      d_wrapped_x, d_wrapped_y, d_wrapped_z,
      d_element_ids, d_atom_bin,
      d_bin_offsets, d_bin_indices,
      gpu_lattice, gpu_grid,
      cutoff_sq, d_bond_cutoffs_sq, static_cast<int>(num_elements),
      ignore_periodic_self_interactions, static_cast<int>(atom_count),
      d_distance_counter, d_bond_counter,
      nullptr, nullptr
  );
  ...
  hipLaunchKernelGGL(distance_kernel<true>, grid_size, block_size, 0, 0,
      d_wrapped_x, d_wrapped_y, d_wrapped_z,
      d_element_ids, d_atom_bin,
      d_bin_offsets, d_bin_indices,
      gpu_lattice, gpu_grid,
  ...
  for (size_t k = 0; k < h_distance_count; ++k) {
    const auto &d = host_distances[k];
    int type_A = element_ids[d.from];
    int type_B = element_ids[d.to];
    out_distances[type_A][type_B].push_back(d.distance);
    if (d.from != d.to && type_A != type_B) {
      out_distances[type_B][type_A].push_back(d.distance);
    }
  }

  for (size_t k = 0; k < h_bond_count; ++k) {
    const auto &b = host_bonds[k];
    out_graph.addDirectedEdge(b.from, b.to, b.distance, correlation::math::Vector3<double>(b.r_x, b.r_y, b.r_z));
    if (b.from != b.to) {
      out_graph.addDirectedEdge(b.to, b.from, b.distance, correlation::math::Vector3<double>(-b.r_x, -b.r_y, -b.r_z));
    }
  }
}
