...
struct ThreadLocalDistances {
  ...
  ThreadLocalDistances(size_t num_elements, size_t atom_count)
      : distance_tensor_local(num_elements, std::vector<std::vector<double>>(num_elements)),
...
void DistanceCalculator::compute(const correlation::core::Cell &cell, double cutoff_sq,
  ...
  tbb::enumerable_thread_specific<ThreadLocalDistances> ets(num_elements, atom_count);
      ...
      for (int dx = -max_dx; dx <= max_dx; ++dx) {
        ...
        for (int dy = -max_dy; dy <= max_dy; ++dy) {
          ...
          for (int dz = -max_dz; dz <= max_dz; ++dz) {
            ...
            correlation::math::Vector3<double> disp =
                lattice[0] * shift_x + lattice[1] * shift_y + lattice[2] * shift_z;
      ...
      if (c_count > 0) {
        ...
        for (size_t k = 0; k < c_count; ++k) {
          ...
          if (d_sq <= max_bond_dist_sq) {
            correlation::math::Vector3<double> r_ij = {soa_x[k] - ax, soa_y[k] - ay, soa_z[k] - az};
  ...
  for (auto &local_results : ets) {
    for (size_t i = 0; i < num_elements; ++i) {
      for (size_t j = 0; j < num_elements; ++j) {
        out_distances[i][j].insert(out_distances[i][j].end(), local_results.distance_tensor_local[i][j].begin(),
                                   local_results.distance_tensor_local[i][j].end());
      }
    }
    for (size_t i = 0; i < atom_count; ++i) {
      for (const auto &neighbor : local_results.neighbor_list_local[i]) {
        out_graph.addDirectedEdge(i, neighbor.index, neighbor.distance, neighbor.r_ij);
      }
    }
  }
}
