...
void StructureFactorCalculator::calculateFrame(DistributionFunctions &df,
  ...
  // where M is the lattice matrix. The reciprocal vectors are b_i = 2*pi * (M^{-T})_i.
  // We extract each reciprocal basis vector from the transpose of the inverse.
  // -----------------------------------------------------------------------
  ...
  for (int h = -hmax; h <= hmax; ++h) {
    for (int k = -kmax; k <= kmax; ++k) {
      for (int l = -lmax; l <= lmax; ++l) {
        if (h == 0 && k == 0 && l == 0) continue; // skip Gamma point
  ...
  std::mutex bin_mutex;
  tbb::enumerable_thread_specific<std::pair<std::vector<double>, std::vector<size_t>>> local_ets(
      [&] { return std::make_pair(std::vector<double>(num_q_bins, 0.0), std::vector<size_t>(num_q_bins, 0)); });
  ...
  tbb::parallel_for(
    ...
    [&](const tbb::blocked_range<size_t> &range) {
      ...
      for (size_t qi = range.begin(); qi != range.end(); ++qi) {
        ...
        simd_utils::complex_exp_sum(q.qx, q.qy, q.qz, xs.data(), ys.data(), zs.data(), N, cos_sum, sin_sum);
        const double sq = (cos_sum * cos_sum + sin_sum * sin_sum) / static_cast<double>(N);
  ...
  df.addHistogram("S(Q)_pw", std::move(s_q_hist));
}
