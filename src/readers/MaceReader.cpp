/**
 * @file MaceReader.cpp
 * @brief Implementation of the MACE Extended XYZ reader.
...
struct MaceParser {
  ...
  MaceParser(const char *data, size_t total_size) : data(data), total_size(total_size) {}
  ...
  int parseAtomCount() {
    ...
    if (num_atoms <= 0) {
      throw std::runtime_error("Invalid MACE file: non-positive atom count: " + atom_count_str);
    }
  ...
  void skipCommentLine() {
    if (offset >= total_size) {
      throw std::runtime_error("Invalid MACE file: missing comment line after atom count");
    }
    size_t line_end = offset;
    while (line_end < total_size && data[line_end] != '\n' && data[line_end] != '\r') {
      line_end++;
  ...
  void skipCoordinateLines(int num_atoms) {
    for (int atom_idx = 0; atom_idx < num_atoms; ++atom_idx) {
      if (offset >= total_size) {
        throw std::runtime_error("Invalid MACE file: unexpected EOF while reading atom lines");
      }
      size_t line_end = offset;
      while (line_end < total_size && data[line_end] != '\n' && data[line_end] != '\r') {
        line_end++;
  ...
  std::vector<size_t> findFrameOffsets(const std::function<void(float, const std::string &)> &progress_callback) {
    ...
    while (true) {
      ...
      if (progress_callback && total_size > 0 && (frame_offsets.size() % 100 == 0)) {
        ...
        progress_callback(progress, "Indexing MACE frames...");
      }
    }
...
void parseAtomLine(const std::string &line, const MaceReader::CommentData &comm_data, correlation::core::Cell &cell) {
  ...
  if (std::cmp_less_equal(tokens.size(), max_idx)) {
    throw std::runtime_error("Invalid MACE file: malformed atom line: " + line);
  }
...
correlation::core::Cell MaceReader::parseMaceFrame(const char *data, size_t size) {
  ...
  if (num_atoms <= 0) {
    throw std::runtime_error("Invalid MACE file: non-positive atom count: " + line);
  }
  ...
  for (int i = 0; i < num_atoms; ++i) {
    if (!std::getline(stream, line)) {
      throw std::runtime_error("Invalid MACE file: unexpected EOF while reading atom " + std::to_string(i + 1));
    }
...
void MaceReader::parseLattice(const std::string &comment, CommentData &data) {
...
void MaceReader::parseEnergy(const std::string &comment, CommentData &data) {
  const std::vector<std::string> energy_keys = {
      "mace_energy=", "MACE_energy=", "free_energy=", "energy=", "dft_energy=", "Energy="};
  ...
  for (const auto &key : energy_keys) {
    ...
    try {
      ...
      std::cerr << "Warning: Failed to parse MACE energy value '" << val_str << "': " << err.what() << '\n';
    }
  }
}

void MaceReader::parseProperties(const std::string &comment, CommentData &data) {
...
void MaceReader::parsePropertiesParts(const std::vector<std::string> &parts, CommentData &data) {
  int col_index = 0;
  data.species_col = -1;
  data.pos_x_col = -1;
  ...
  for (size_t part_idx = 0; part_idx + 2 < parts.size(); part_idx += 3) {
    ...
    try {
      ...
      std::cerr << "Warning: Failed to parse column count '" << parts.at(part_idx + 2)
                << "' in MACE Properties header: " << err.what() << '\n';
    }
    ...
    } else if (name == "mace_forces" || name == "MACE_forces" || name == "forces") {
...
MaceReader::CommentData MaceReader::parseCommentLine(const std::string &comment) {
  CommentData data;
...
correlation::core::Cell MaceReader::readStructure(const std::string &filename,
                                                  std::function<void(float, const std::string &)> progress_callback) {
  auto traj = readTrajectory(filename, std::move(progress_callback));
  if (traj.getFrameCount() == 0) {
    throw std::runtime_error("No structure found in MACE file: " + filename);
  }
  return traj.getFrame(traj.getFrameCount() - 1);
}

correlation::core::Trajectory
MaceReader::readTrajectory(const std::string &filename,
                           std::function<void(float, const std::string &)> progress_callback) {
  if (progress_callback) {
    progress_callback(0.0F, "Reading MACE file...");
  }
  ...
  MaceParser parser(data, total_size);
  std::vector<size_t> frame_offsets = parser.findFrameOffsets(progress_callback);

  if (frame_offsets.empty()) {
    throw std::runtime_error("No frames found in MACE file: " + filename);
  }

  frame_offsets.push_back(parser.offset);

  if (progress_callback) {
    progress_callback(1.0F, "MACE file loaded.");
  }

  auto parser_func = [](const char *data_begin, size_t data_size) { return parseMaceFrame(data_begin, data_size); };
