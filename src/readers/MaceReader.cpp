...
void parseAtomLine(const std::string &line, const MaceReader::CommentData &comm_data,
...
correlation::core::Cell MaceReader::parseMaceFrame(const char *data, size_t size) {
  ...
  for (int i = 0; i < num_atoms; ++i) {
    if (!std::getline(stream, line)) {
      throw std::runtime_error("Invalid MACE file: unexpected EOF while reading atom " +
                               std::to_string(i + 1));
    }
...
MaceReader::readStructure(const std::string &filename,
...
MaceReader::readTrajectory(const std::string &filename,
  ...
  auto parser_func = [](const char *data_begin, size_t data_size) {
    return parseMaceFrame(data_begin, data_size);
  };
