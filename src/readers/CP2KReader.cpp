...
namespace {
bool registered =
    ReaderFactory::instance().registerReader(std::make_unique<CP2KReader>());
}

correlation::core::Cell CP2KReader::readStructure(
...
correlation::core::Trajectory CP2KReader::readTrajectory(
    const std::string &filename,
    std::function<void(float, const std::string &)> progress_callback) {
