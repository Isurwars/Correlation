...
correlation::core::Cell GromacsReader::readStructure(
    ...
    if (progress_callback) progress_callback(0.1f, "Reading GROMACS structure...");

    std::string line;
    
    // Line 1: Title/Comment
    if (!std::getline(file, line)) throw std::runtime_error("Invalid GROMACS file: empty");
    
    // Line 2: Number of atoms
    if (!std::getline(file, line)) throw std::runtime_error("Invalid GROMACS file: missing atom count");
    int num_atoms = 0;
    try {
    ...
    cell.atoms().reserve(num_atoms);

    // Lines 3 to num_atoms + 2: Atoms
    // Format: 5pos 5res 5atom 5id 8x 8y 8z 8vx 8vy 8vz (last 3 optional)
    for (int i = 0; i < num_atoms; ++i) {
        if (!std::getline(file, line)) throw std::runtime_error("Invalid GROMACS file: unexpected EOF");
        
        if (line.length() < 44) continue; // Basic coordinate check
        ...
        if (symbol == "OW" || symbol == "HW") symbol = symbol.substr(0, 1);
        ...
        if (progress_callback && i % 1000 == 0) {
            progress_callback(0.1f + 0.8f * (static_cast<float>(i) / num_atoms), "Parsing atoms...");
        }
    }
    ...
    if (std::getline(file, line)) {
        ...
        double bx, by, bz;
        if (ss >> bx >> by >> bz) {
            cell.setBox(bx * 10.0, by * 10.0, bz * 10.0);
        }
    }

    if (progress_callback) progress_callback(1.0f, "GROMACS file loaded.");
    return cell;
}
