/**
 * @file NequipReader.cpp
 * @brief Implementation of the NequIP Extended XYZ reader.
...
struct NequipParser {
  ...
  NequipParser(const char *data, size_t total_size) : data(data), total_size(total_size) {}
  ...
  int parseAtomCount() {
    ...
    try {
      ...
      throw std::runtime_error("Invalid NequIP file: expected atom count, got: " + atom_count_str);
    }

    if (num_atoms <= 0) {
      throw std::runtime_error("Invalid NequIP file: non-positive atom count: " + atom_count_str);
    }
  ...
  void skipCommentLine() {
    if (offset >= total_size) {
      throw std::runtime_error("Invalid NequIP file: missing comment line after atom count");
    }
    size_t line_end = offset;
    while (line_end < total_size && data[line_end] != '\n' && data[line_end] != '\r') {
      line_end++;
  ...
  void skipCoordinateLines(int num_atoms) {
    for (int atom_idx = 0; atom_idx < num_atoms; ++atom_idx) {
      if (offset >= total_size) {
        throw std::runtime_error("Invalid NequIP file: unexpected EOF while reading atom lines");
      }
      size_t line_end = offset;
      while (line_end < total_size && data[line_end] != '\n' && data[line_end] != '\r') {
        line_end++;
  ...
  findFrameOffsets(const std::function<void(float, const std::string &)> &progress_callback) {
    ...
    while (true) {
      ...
      if (progress_callback && total_size > 0 && (frame_offsets.size() % 100 == 0)) {
        ...
        progress_callback(progress, "Indexing NequIP frames...");
      }
    }
...
void parseAtomLine(const std::string &line, const NequipReader::CommentData &comm_data,
  ...
  if (std::cmp_less_equal(tokens.size(), max_idx)) {
    throw std::runtime_error("Invalid NequIP file: malformed atom line: " + line);
  }
  ...
  try {
    ...
    throw std::runtime_error("Invalid NequIP file: invalid coordinates or forces: " + line);
  }
}

} // namespace

correlation::core::Cell NequipReader::parseNequipFrame(const char *data, size_t size) {
  ...
  if (line.empty()) {
    throw std::runtime_error("Invalid NequIP file: empty frame");
  }
  ...
  try {
    ...
    throw std::runtime_error("Invalid NequIP file: expected atom count, got: " + line);
  }

  if (num_atoms <= 0) {
    throw std::runtime_error("Invalid NequIP file: non-positive atom count: " + line);
  }
  ...
  for (int i = 0; i < num_atoms; ++i) {
    if (!std::getline(stream, line)) {
      throw std::runtime_error("Invalid NequIP file: unexpected EOF while reading atom " +
                               std::to_string(i + 1));
    }
...
void NequipReader::parseLattice(const std::string &comment, CommentData &data) {
...
void NequipReader::parseEnergy(const std::string &comment, CommentData &data) {
  const std::vector<std::string> energy_keys = {
      "nequip_energy=", "NequIP_energy=", "free_energy=", "energy=", "dft_energy=", "Energy="};
  ...
  for (const auto &key : energy_keys) {
    ...
    try {
      ...
      std::cerr << "Warning: Failed to parse NequIP energy value '" << val_str
                << "': " << err.what() << '\n';
    }
  }
}

void NequipReader::parseProperties(const std::string &comment, CommentData &data) {
...
void NequipReader::parsePropertiesParts(const std::vector<std::string> &parts, CommentData &data) {
  int col_index = 0;
  data.species_col = -1;
  data.pos_x_col = -1;
  ...
  for (size_t part_idx = 0; part_idx + 2 < parts.size(); part_idx += 3) {
    ...
    try {
      ...
      std::cerr << "Warning: Failed to parse column count '" << parts.at(part_idx + 2)
                << "' in NequIP Properties header: " << err.what() << '\n';
    }
    ...
    } else if (name == "nequip_forces" || name == "NequIP_forces" || name == "forces") {
...
NequipReader::CommentData NequipReader::parseCommentLine(const std::string &comment) {
  CommentData data;
...
NequipReader::readStructure(const std::string &filename,
                            std::function<void(float, const std::string &)> progress_callback) {
  auto traj = readTrajectory(filename, std::move(progress_callback));
  if (traj.getFrameCount() == 0) {
    throw std::runtime_error("No structure found in NequIP file: " + filename);
  }
  return traj.getFrame(traj.getFrameCount() - 1);
}

correlation::core::Trajectory
NequipReader::readTrajectory(const std::string &filename,
                             std::function<void(float, const std::string &)> progress_callback) {
  if (progress_callback) {
    progress_callback(0.0F, "Reading NequIP file...");
  }
