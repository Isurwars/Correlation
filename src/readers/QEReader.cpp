...
namespace {
bool registered =
    ReaderFactory::instance().registerReader(std::make_unique<QEReader>());
}

correlation::core::Cell QEReader::readStructure(
...
correlation::core::Trajectory QEReader::readTrajectory(
    const std::string &filename,
    std::function<void(float, const std::string &)> progress_callback) {
