...
- [ ] **VASP (`POSCAR`, `CONTCAR`, `XDATCAR`)**: Essential for the inorganic materials, ceramics, and solid-state physics communities.
- [ ] **GROMACS (`.gro`, `.xtc`, `.trr`)**: Opens `Correlation` to the soft-matter and biophysics communities.
- [ ] **Protein Data Bank (`.pdb`)**: The universal standard for biological and organic structures.
...
- [ ] **Steinhardt Bond-Orientational Parameters ($Q_4, Q_6, W_6$)**: The gold standard for distinguishing rotationally invariant local structures in liquids vs amorphous/crystalline solids.
...
- [ ] **Hydrogen Bond Analysis**: Using geometric criteria (distance + angle cutoffs) to count hydrogen bonds, and dynamical H-bond autocorrelation functions to measure their lifetimes.
...
- [ ] **Plugin/Factory Pattern for Readers & Calculators**: Refactor the architecture so adding a new `Reader` or `Calculator` only requires dropping in a new class that automatically registers itself with the GUI, without modifying the core UI and `AppController`.
- [ ] **Explicit SIMD Vectorization**: Explore using explicit SIMD (AVX2/AVX-512) for the inner loops of distance calculations to achieve even higher performance on top of the existing Intel TBB multithreading.
- [ ] **Automated Release Pipelines**: Set up GitHub Actions to automatically compile binaries for Windows/Linux/macOS, generate installers, and update the AUR package upon a new GitHub Release.
