...
TEST_F(SpecialFunctionsTests, SphLegendreBoundaryCases) {
  // Out of bounds m
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 1, .order = -1}, static_cast<real_t>(0.5))), 0.0, 1e-5);
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 2, .order = 3}, static_cast<real_t>(0.5))), 0.0, 1e-5);
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 0, .order = 1}, static_cast<real_t>(0.5))), 0.0, 1e-5);
}
...
TEST_F(SpecialFunctionsTests, SphLegendreAnalyticValues) {
  ...
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 0, .order = 0}, theta)), y00_expected, 1e-4);

  // l=1, m=0: Y_1^0 = sqrt(3 / (4 * pi)) * cos(theta)
  double const y10_expected = std::sqrt(3.0 / (4.0 * M_PI)) * std::cos(0.5);
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 1, .order = 0}, theta)), y10_expected, 1e-4);
  ...
  EXPECT_NEAR(static_cast<double>(sph_legendre({.degree = 1, .order = 1}, theta)), y11_expected, 1e-4);
}
...
TEST_F(SpecialFunctionsTests, SphLegendreBatchEquivalence) {
  ...
  for (const auto &[degree, order] : l_m_pairs) {
    ...
    for (size_t i = 0; i < count; ++i) {
      real_t const expected = sph_legendre({.degree = degree, .order = order}, angles[i]);
