...
TEST_F(SmoothingTests, GenerateKernelNormalizesAndCalculatesCorrectly) {
  ...
  // Triweight
  auto k_tri = generateKernel({
      .size = size,
      .bin_width = bin_width,
      .sigma = sigma,
      .type = KernelType::Triweight,
  });
  ...
  // Bump
  auto k_bump = generateKernel({
      .size = size,
      .bin_width = bin_width,
      .sigma = sigma,
      .type = KernelType::Bump,
  });
  ...
  // Epanechnikov
  auto k_epan = generateKernel({
      .size = size,
      .bin_width = bin_width,
      .sigma = sigma,
      .type = KernelType::Epanechnikov,
  });
  ...
  // Cosine
  auto k_cos = generateKernel({
      .size = size,
      .bin_width = bin_width,
      .sigma = sigma,
      .type = KernelType::Cosine,
  });
  ...
  // Biweight
  auto k_biw = generateKernel({
      .size = size,
      .bin_width = bin_width,
      .sigma = sigma,
      .type = KernelType::Biweight,
  });
...
TEST_F(SmoothingTests, KernelSmoothingBoundaryInputsAndErrors) {
  ...
  auto empty_res = KernelSmoothing(empty_y, {
                                                .bin_width = 0.1,
                                                .sigma = 0.5,
                                                .type = KernelType::Gaussian,
                                            });
  ...
  EXPECT_THROW(KernelSmoothing(y_values,
                               {
                                   .bin_width = 0.0,
                                   .sigma = 0.5,
                                   .type = KernelType::Gaussian,
...
TEST_F(SmoothingTests, KernelSmoothingComputesConvolutions) {
  ...
  auto smoothed = KernelSmoothing(y_values, {
                                                .bin_width = bin_width,
                                                .sigma = sigma,
                                                .type = KernelType::Gaussian,
                                            });
...
TEST_F(SmoothingTests, CompatibilityOverloadDerivesBinWidth) {
  ...
  auto smoothed_r = KernelSmoothing(r_values, y_values,
                                    {
                                        .sigma = sigma,
                                        .type = KernelType::Gaussian,
                                    });
  auto smoothed_dx = KernelSmoothing(y_values, {
                                                   .bin_width = 0.2,
                                                   .sigma = sigma,
                                                   .type = KernelType::Gaussian,
                                               });
  ...
  EXPECT_TRUE(KernelSmoothing(r_bad, y_values,
                              {
                                  .sigma = sigma,
                                  .type = KernelType::Gaussian,
                              })
  ...
  EXPECT_TRUE(KernelSmoothing(r_short, y_short,
                              {
                                  .sigma = sigma,
                                  .type = KernelType::Gaussian,
                              })
