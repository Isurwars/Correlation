...
inline real_t vecNorm(const math::Vector3<real_t> &vec) noexcept {
...
protected:
  [[nodiscard]] real_t radius() const noexcept { return radius_; }
  [[nodiscard]] real_t boxSize() const noexcept { return box_size_; }

  [[nodiscard]] correlation::core::Cell createRing(size_t num_atoms) const {
...
TEST_P(RingCellTests, GeometryAndBondDistances) {
  ...
  for (size_t i = 0; i < numAtoms; ++i) {
    ...
    real_t const deltaX = atom.position().x() - mid;
    real_t const deltaY = atom.position().y() - mid;
    real_t const deltaZ = atom.position().z() - mid;
    real_t const dist_from_center = std::sqrt(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ);
    EXPECT_NEAR(dist_from_center, radius(), 1e-4);
  }

  // Check distance between adjacent ring atoms
  for (size_t i = 0; i < numAtoms; ++i) {
    size_t const next_idx = (i + 1) % numAtoms;
    const auto &atom1 = cell.atoms()[i];
    const auto &atom2 = cell.atoms()[next_idx];
    real_t const dist = vecNorm(atom1.position() - atom2.position());
...
TEST_P(RingCellTests, MotifFinderRingDetection) {
  ...
  core::NeighborGraph graph(numAtoms);
  for (size_t i = 0; i < numAtoms; ++i) {
    size_t const next_idx = (i + 1) % numAtoms;
    const auto &atom1 = cell.atoms()[i];
    const auto &atom2 = cell.atoms()[next_idx];
    real_t const dist = vecNorm(atom1.position() - atom2.position());
    graph.addDirectedEdge(i, next_idx, dist, {0, 0, 0});
    graph.addDirectedEdge(next_idx, i, dist, {0, 0, 0});
  }
