...
TEST(DistanceCalculatorTests, ThrowsOnInvalidCutoff) {
  ...
  EXPECT_THROW(DistanceCalculator::compute(cell, -1.0, bond_cutoffs, true, out_distances, out_graph),
               std::invalid_argument);
  EXPECT_THROW(DistanceCalculator::compute(cell, 0.0, bond_cutoffs, true, out_distances, out_graph),
               std::invalid_argument);
}
