...
class NequipReaderTests : public ::testing::Test {
public:
  struct TempFileSpec {
    std::string filename;
    std::string content;
...
protected:
  void SetUp() override {
    test_dir_ = std::filesystem::temp_directory_path() / "correlation_nequip_tests";
...
TEST_F(NequipReaderTests, ReadSingleFrameNequip) {
  std::string const nequip_content =
      "2\n"
      "Lattice=\"5.0 0.0 0.0 0.0 6.0 0.0 0.0 0.0 7.0\" Properties=species:S:1:pos:R:3:nequip_forces:R:3 "
      "nequip_energy=-25.432\n"
      "Si 0.1 0.2 0.3 0.01 -0.02 0.03\n"
      "O 1.5 2.5 3.5 -0.04 0.05 -0.06\n";

  auto file_path = createTempFile({.filename = "single_frame.nequip", .content = nequip_content});
...
TEST_F(NequipReaderTests, ReadMultiFrameTrajectory) {
  std::string const traj_content =
      "2\n"
      "Lattice=\"10.0 0.0 0.0 0.0 10.0 0.0 0.0 0.0 10.0\" Properties=species:S:1:pos:R:3:nequip_forces:R:3 "
      "nequip_energy=-100.5\n"
  ...
  auto file_path = createTempFile({.filename = "trajectory.nequip", .content = traj_content});
...
TEST_F(NequipReaderTests, ReadStructureReturnsLastFrame) {
...
TEST_F(NequipReaderTests, MalformedHeaderThrows) {
...
TEST_F(NequipReaderTests, ReaderFactoryRegistration) {
  auto *reader = correlation::readers::ReaderFactory::instance().getReaderForExtension(".nequip");
  ASSERT_NE(reader, nullptr);
  EXPECT_EQ(reader->getName(), "NequIP Reader");
