...
TEST_F(SlintSvgIntegrationTests, LoadsBasicSvgFromEmbeddedData) {
    std::string svg = R"(<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><circle cx="50" cy="50" r="40" stroke="green" stroke-width="4" fill="yellow" /></svg>)";
...
TEST_F(SlintSvgIntegrationTests, LoadsSvgWithGradientFromEmbeddedData) {
    std::string svg = R"lit(<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><defs><linearGradient id="g1"><stop offset="0%" stop-color="red"/><stop offset="100%" stop-color="blue"/></linearGradient></defs><rect width="120" height="120" fill="url(#g1)"/></svg>)lit";
    auto img = slint::private_api::load_image_from_embedded_data(
          std::span<const uint8_t>(reinterpret_cast<const uint8_t *>(svg.data()), svg.size()), "svg");
...
TEST_F(SlintSvgIntegrationTests, FailsWithDotExtension) {
    std::string svg = R"(<svg xmlns="http://www.w3.org/2000/svg" width="50" height="50"><circle cx="25" cy="25" r="20"/></svg>)";
    auto img = slint::private_api::load_image_from_embedded_data(
          std::span<const uint8_t>(reinterpret_cast<const uint8_t *>(svg.data()), svg.size()), ".svg");
...
TEST_F(SlintSvgIntegrationTests, DataUriLoadingFailsAsExpected) {
    ...
    std::string const b64 = "PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIj48Y2lyY2xlIGN4PSI1MCIgY3k9IjUwIiByPSI0MCIgc3Ryb2tlPSJncmVlbiIgc3Ryb2tlLXdpZHRoPSI0IiBmaWxsPSJ5ZWxsb3ciIC8+PC9zdmc+";
