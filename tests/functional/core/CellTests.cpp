...
TEST_F(CellFunctionalTests, BuildBCCLatticeAndVerifyDensity) {
  ...
  cell.addAtom("Fe",
               correlation::math::Vector3<real_t>(lattice_parameter * static_cast<real_t>(0.5),
                                                  lattice_parameter * static_cast<real_t>(0.5),
                                                  lattice_parameter * static_cast<real_t>(0.5)));

  EXPECT_EQ(cell.atomCount(), 2);
  EXPECT_NEAR(cell.volume(), std::pow(lattice_parameter, 3),
              correlation::is_single_precision ? 1e-5 : 1e-6);
  ...
  EXPECT_NEAR(cell.atoms()[1].position().x(), lattice_parameter * 0.5,
...
TEST_F(CellFunctionalTests, BuildFCCLatticeAndVerifyPBCDistances) {
  ...
  cell.addAtom("Al",
               correlation::math::Vector3<real_t>(0.0, lattice_parameter * static_cast<real_t>(0.5),
                                                  lattice_parameter * static_cast<real_t>(0.5)));
  cell.addAtom("Al",
               correlation::math::Vector3<real_t>(lattice_parameter * static_cast<real_t>(0.5), 0.0,
                                                  lattice_parameter * static_cast<real_t>(0.5)));
  cell.addAtom(
      "Al", correlation::math::Vector3<real_t>(lattice_parameter * static_cast<real_t>(0.5),
...
TEST_F(CellFunctionalTests, VerifyWaterMoleculePBCStability) {
  ...
  cell.addAtom("H", correlation::math::Vector3<real_t>(static_cast<real_t>(10.0) + oh_dist,
                                                       ...
                                                       static_cast<real_t>(10.0)));
  cell.addAtom("H", correlation::math::Vector3<real_t>(
                        static_cast<real_t>(10.0) + oh_dist * std::cos(hoh_angle_rad),
  ...
  EXPECT_NEAR(correlation::math::norm(image_1), oh_dist,
              correlation::is_single_precision ? 1e-5 : 1e-6);
  EXPECT_NEAR(correlation::math::norm(image_2), oh_dist,
              correlation::is_single_precision ? 1e-5 : 1e-6);
  ...
  real_t const cos_theta = correlation::math::dot(image_1, image_2) /
                           (correlation::math::norm(image_1) * correlation::math::norm(image_2));
  real_t const calc_angle =
      std::acos(std::clamp(cos_theta, static_cast<real_t>(-1.0), static_cast<real_t>(1.0)));
