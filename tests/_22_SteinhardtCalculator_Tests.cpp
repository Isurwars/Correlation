...
class _24_SteinhardtCalculator_Tests : public ::testing::Test {
protected:
  void checkOutputs(const std::map<std::string, Histogram>& hists, double expected_Q4, double expected_Q6, double expected_W6_hat) {
    ...
    size_t q4_bins = 100; double dQ = 1.0 / q4_bins;
    size_t w6_bins = 100; double W_min = -0.2; double W_max = 0.2; double dW = (W_max - W_min) / w6_bins;

    for (size_t b = 0; b < q4_bins; ++b) {
      if (hist_Q4[b] > 0) q4_val = (b + 0.5) * dQ;
      if (hist_Q6[b] > 0) q6_val = (b + 0.5) * dQ;
    }
    for (size_t b = 0; b < w6_bins; ++b) {
      if (hist_W6[b] > 0) w6_val = W_min + (b + 0.5) * dW;
    }
...
TEST_F(_24_SteinhardtCalculator_Tests, SimpleCubic) {
...
TEST_F(_24_SteinhardtCalculator_Tests, BCC) {
  Cell cell({1.0, 1.0, 1.0, 90.0, 90.0, 90.0});
  cell.addAtom("Ar", {0.0, 0.0, 0.0});
  cell.addAtom("Ar", {0.5, 0.5, 0.5});

  StructureAnalyzer analyzer(cell, 1.1, {{1.1 * 1.1}}, false); // dist = sqrt(0.75) ~ 0.866 and 1.0
  auto hists = SteinhardtCalculator::calculate(cell, &analyzer);

  checkOutputs(hists, 0.036, 0.511, 0.013);
}

TEST_F(_24_SteinhardtCalculator_Tests, FCC) {
  Cell cell({1.0, 1.0, 1.0, 90.0, 90.0, 90.0});
  cell.addAtom("Ar", {0.0, 0.0, 0.0});
  ...
  StructureAnalyzer analyzer(cell, 0.8, {{0.8 * 0.8}}, false); // dist = sqrt(0.5) ~ 0.707
  auto hists = SteinhardtCalculator::calculate(cell, &analyzer);

  checkOutputs(hists, 0.191, 0.575, -0.013); // W6 for FCC is approx -0.013
}

TEST_F(_24_SteinhardtCalculator_Tests, Icosahedral) {
  ...
  // Edge length is ~1.05. Using cutoff 1.02 ensures surface atoms only see center.
  // Thus they will have 1 neighbor, Ql=1.0, and be excluded from histogram!
  StructureAnalyzer analyzer(cell, 1.02, {{1.02 * 1.02}}, true); 
  auto hists = SteinhardtCalculator::calculate(cell, &analyzer);

  checkOutputs(hists, 0.000, 0.663, -0.169); // W6_hat for Icosahedral is approx -0.1697
}
