...
TEST(AtomTests, DefaultConstructorSetsBasicProperties) {
  Atom atom;
  Atom_Img img = atom.getImage();

  std::array<double, 3> expected_pos{0.0, 0.0, 0.0};
  EXPECT_EQ(img.position, expected_pos);

  Atom another_atom;
  EXPECT_NE(atom.getID(), another_atom.getID());
}
...
TEST(AtomTests, DistanceCalculationIsAccurate) {
  Atom a1("H", {0.0, 0.0, 0.0});
  Atom a2("H", {3.0, 4.0, 0.0});
...
TEST(AtomTests, BondAngleCalculationHandlesBasicCases) {
  ...
  Atom a("H", {1.0, 0.0, 0.0});
  Atom b("H", {0.0, 1.0, 0.0});

  double angle = center.getAngle(a.getImage(), b.getImage());
...
TEST(AtomTests, AddBondedAtomUpdatesList) {
  Atom atom;
  Atom_Img bonded_img;
  bonded_img.atom_id = 123;

  atom.addBondedAtom(bonded_img);
  std::vector<int> bonded_ids = atom.getBondedAtomsID();
