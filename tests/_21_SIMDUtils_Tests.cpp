...
TEST_F(_21_SIMDUtils_Tests, SincIntegralMatchesScalar) {
  // Test both exact multiples of SIMD width and non-multiples to hit the tail logic
  ...
  for (size_t size : sizes) {
    ...
    double actual_acc = simd_utils::sinc_integral(Q, integrand.data(), rbins.data(), scratch.data(), size);
...
TEST_F(_21_SIMDUtils_Tests, ComputeDsqBlockMatchesScalar) {
  ...
  for (size_t size : sizes) {
    ...
    simd_utils::PositionBlock block{bx.data(), by.data(), bz.data(), size};
    ...
    for (size_t k = 0; k < size; ++k) {
      expected_dsq[k] = simd_utils::dist_sq_scalar(ax, ay, az, bx[k], by[k], bz[k]);
    }

    // Call SIMD implementation
    simd_utils::compute_dsq_block(ax, ay, az, block, actual_dsq.data());
...
TEST_F(_21_SIMDUtils_Tests, NormalizeRDFBinsMatchesScalar) {
  ...
  for (size_t size : sizes) {
    ...
    // Call SIMD implementation
    simd_utils::normalize_rdf_bins(H.data(), rbins.data(), g_norm, inv_Ni_dr, inv_Nj_dr, pi4_rho_j,
...
TEST_F(_21_SIMDUtils_Tests, ScaleBinsMatchesScalar) {
  ...
  for (size_t size : sizes) {
    ...
    simd_utils::scale_bins(data.data(), scale_factor, size);
...
TEST_F(_21_SIMDUtils_Tests, DotBlockMatchesScalar) {
  ...
  for (size_t size : sizes) {
    ...
    simd_utils::dot_block(v1x, v1y, v1z, v2x.data(), v2y.data(), v2z.data(), actual_out.data(), size);
