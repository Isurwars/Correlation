...
struct SvgComparisonRenderer {
  ...
  SvgComparisonRenderer(const std::vector<LabeledHistogram> &datasets_ref,
                        const std::string &partial_key_ref, const PlotConfig &config_ref,
                        const HoverInfo &hover_ref)
      : datasets(&datasets_ref), partial_key(&partial_key_ref), config(&config_ref),
        hover(&hover_ref) {}
  ...
  [[nodiscard]] bool computeDifferenceCurve(std::vector<real_t> &diff_x,
                                            std::vector<real_t> &diff_y) const {
    if (!config->show_difference_curve || datasets->size() < 2) {
      return false;
    }
    const auto &ref_ds = datasets->at(0);
    const auto &tgt_ds = datasets->at(1);
    const auto &ref_partials = ref_ds.hist->smoothed_partials.empty()
                                   ? ref_ds.hist->partials
                                   : ref_ds.hist->smoothed_partials;
    const auto &tgt_partials = tgt_ds.hist->smoothed_partials.empty()
                                   ? tgt_ds.hist->partials
                                   : tgt_ds.hist->smoothed_partials;
  ...
  void drawDatasetPolyline(const LabeledHistogram &dataset, std::size_t color_idx) {
    ...
    const std::string col = dataset.style.color_hex.empty()
                                ? detail::color(color_idx, config->palette)
                                : dataset.style.color_hex;
    const float stroke_w = dataset.style.stroke_width > 0.0F
                               ? dataset.style.stroke_width
                               : static_cast<float>(config->line_width);
    ...
    if (config->show_legend) {
      ...
      for (auto [label, color] : std::views::reverse(legend)) {
        svg << std::format("  <line x1=\"{:.1f}\" y1=\"{:.1f}\" x2=\"{:.1f}\" y2=\"{:.1f}\" "
                           ...
                           legend_x - static_cast<real_t>(10.0), legend_y, color);
  ...
  void
  drawTooltipBox(const TooltipPosition &pos, const std::string &best_label,
    ...
    svg << renderTextAsPath(header_txt, tooltip_x + static_cast<real_t>(12.0),
                            tooltip_y + static_cast<real_t>(24.0),
    ...
    for (const auto &[name, val, col] : hover_values) {
      svg << std::format("  <circle cx=\"{:.1f}\" cy=\"{:.1f}\" r=\"5\" fill=\"{}\"/>\n",
                         tooltip_x + static_cast<real_t>(18.0), cur_y - static_cast<real_t>(5.0),
                         col);
  ...
  void drawHoverTooltip() {
    ...
    auto hover_values =
        collectHoverValuesAndDrawMarkers(best_idx, best_label, sx_data, snapped_sy_data);
