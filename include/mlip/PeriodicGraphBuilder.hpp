...
struct PeriodicGraphData {
  ...
  std::vector<int64_t>
      edge_index_flat; /**< [2 * E] Directed edge indices (row 0: src, row 1: dst). */
  std::vector<real_t>
      edge_shifts_flat; /**< [E * 3] Periodic cell displacement integer shift vectors. */
  std::vector<real_t>
      edge_vectors_flat; /**< [E * 3] Cartesian displacement vectors r_ij = r_j + R*s - r_i. */
  std::vector<real_t> edge_distances;                /**< [E] Euclidean edge distances ||r_ij||. */
  std::vector<real_t> edge_spherical_harmonics_flat; /**< [E * (l_max + 1)^2] Equivariant spherical
                                                        harmonics features (if l_max > 0). */
  std::vector<real_t>
      edge_unit_vectors_flat; /**< [E * 3] Normalized displacement vectors r_hat = r_ij / d. */
  ...
  std::vector<real_t> edge_orb_features_flat;    /**< [E * (num_rbf * (l_max + 1)^2)] Fused outer
                                                    product tensor f_cut * (RBF (x) Y_lm). */
  std::vector<int>
      cna_labels; /**< [N] Per-atom Common Neighbor Analysis (CNA) classification label. */
  std::vector<real_t> coordination_desc; /**< [N] Per-atom coordination number embedding. */
  std::vector<real_t>
      ring_desc; /**< [N * max_ring_size] Per-atom ring membership count embedding. */
...
public:
   ...
   * @param[in] l_max Maximum spherical harmonics degree (0..3) to embed (default: 0, no harmonics
   * computed).
  ...
  /**
   * @brief Computes ORB-v3 polynomial cutoff envelope f_c(d) of order p=4 decaying smoothly to 0 at
   * cutoff_radius.
   * @param[in] distance Interatomic distance d.
   * @param[in] cutoff_radius Cutoff radius.
   * @return Envelope value in range [0, 1].
   */
  [[nodiscard]] static real_t computeOrbCutoffEnvelope(real_t distance,
   ...
   */
  [[nodiscard]] static std::vector<real_t> computeBesselBasis(real_t distance, real_t cutoff_radius,
                                                              size_t num_basis);
  ...
  computeOrbBesselBasis(real_t distance, const OrbDescriptorConfig &config = {});
  ...
  /**
   * @brief Evaluates real orthonormal spherical harmonics Y_lm(r) up to l_max <= 3 into a
   * pre-allocated span.
  ...
  /**
   * @brief Evaluates real orthonormal spherical harmonics Y_lm(r) up to l_max <= 3 returning a
   ...
   */
  [[nodiscard]] static std::vector<real_t>
  computeSphericalHarmonics(const correlation::math::Vector3<real_t> &vec, size_t l_max);

  /**
   * @brief Evaluates e3nn component-normalized spherical harmonics Y_lm(r) up to l_max <= 3 into a
   * span.
   * @param[in] unit_vec 3D normalized unit displacement vector.
   * @param[in] l_max Maximum degree (clamped to 3). Total components evaluated: (l_max + 1)^2.
   * @param[out] out Destination span of size at least (l_max + 1)^2.
   */
  static void computeOrbSphericalHarmonics(const correlation::math::Vector3<real_t> &unit_vec,
