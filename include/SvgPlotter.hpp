...
/// Material Design-inspired colour palette (hex strings, inline SVG fill/stroke).
...
inline std::string color(std::size_t i) {
    return kColors[i % kColors.size()];
}

/// Map a value in [data_min, data_max] to SVG coordinate space [svg_min, svg_max].
inline double mapValue(double v, double data_min, double data_max,
                       double svg_min, double svg_max) {
    if (std::abs(data_max - data_min) < 1e-15) return (svg_min + svg_max) / 2.0;
...
inline void appendPolyline(std::ostringstream& svg,
                            ...
                            double plot_x0, double plot_x1,
                            double plot_y0, double plot_y1,
...
inline std::string renderHistogramAsSvg(const Histogram& hist,
    ...
    const auto& partials = hist.smoothed_partials.empty()
                               ? hist.partials
                               : hist.smoothed_partials;
    ...
    if (std::abs(y_max - y_min) < 1e-12) { y_min = 0.0; y_max = 1.0; }
    ...
    for (const auto& [key, ys] : partials) {
        ...
        detail::appendPolyline(svg, xs, ys,
...
/**
 * @brief Convenience overload: picks the best-fit axis labels and title for
 * well-known histogram names used by the Correlation calculators.
 *
 * Supported histogram keys and their labels:
 * | Histogram key | Title         | X label          | Y label   |
 * |---------------|---------------|------------------|-----------|
 * | g(r)          | g(r)          | r (Å)            | g(r)      |
 * | J(r)          | J(r)          | r (Å)            | J(r)      |
 * | G(r)          | G(r)          | r (Å)            | G(r)      |
 * | S(Q)          | S(Q)          | Q (Å⁻¹)         | S(Q)      |
 * | XRD           | XRD Pattern   | 2θ (°)           | Intensity |
 * | PAD           | Plane-Angle   | θ (°)            | P(θ)      |
 * | DAD           | Dihedral Angle| φ (°)            | P(φ)      |
 * | CN            | Coord. Number | CN               | Count     |
 * | VACF          | VACF          | t (fs)           | C(t)      |
 * | VDOS          | VDOS          | ν (THz)          | g(ν)      |
 * | MSD           | MSD           | t (fs)           | MSD (Å²)  |
 * | (other)       | <name>        | x                | y         |
...
inline std::string renderHistogramAsSvg(const Histogram& hist,
    ...
    struct Labels { std::string title, x, y; };

    static const std::map<std::string, Labels> kLabels = {
        {"g(r)",  {"g(r) — Radial Distribution",       "r (Å)",    "g(r)"}},
        {"J(r)",  {"J(r) — Reduced RDF",               "r (Å)",    "J(r)"}},
        {"G(r)",  {"G(r) — Pair Distribution",         "r (Å)",    "G(r)"}},
        {"S(Q)",  {"S(Q) — Structure Factor",          "Q (Å⁻¹)",  "S(Q)"}},
        {"XRD",   {"XRD Pattern",                      "2θ (°)",   "Intensity"}},
        {"PAD",   {"Plane-Angle Distribution",         "θ (°)",    "P(θ)"}},
        {"DAD",   {"Dihedral-Angle Distribution",      "φ (°)",    "P(φ)"}},
        {"CN",    {"Coordination Number Distribution", "CN",       "Count"}},
        {"VACF",  {"Velocity Autocorrelation",         "t (fs)",   "C(t)"}},
        {"VDOS",  {"Vibrational Density of States",    "ν (THz)",  "g(ν)"}},
        {"MSD",   {"Mean Squared Displacement",        "t (fs)",   "MSD (Å²)"}},
    };

    auto it = kLabels.find(name);
    if (it != kLabels.end()) {
        return renderHistogramAsSvg(hist, it->second.title, it->second.x, it->second.y);
    }
    return renderHistogramAsSvg(hist, name, "x", "y");
}
