/**
 * @file NequipReader.hpp
 * @brief Reader for NequIP (Neural Network Potential) Extended XYZ trajectory files.
...
/**
 * @class NequipReader
 * @brief Reads atomic structures and trajectories from NequIP Extended XYZ outputs.
 *
 * NequIP outputs follow the Extended XYZ convention, writing simulation frames with
 * `Lattice="..."`, energy fields (`nequip_energy`, `energy`, `free_energy`), and
 * per-atom properties in `Properties=...` (such as `species:S:1:pos:R:3:nequip_forces:R:3`).
 *
 * When atomic forces (`nequip_forces`, `forces`, `NequIP_forces`) are present, they are
 * captured and stored in the atom's velocity vector for downstream structural and
 * dynamical analysis.
 */
class NequipReader : public BaseReader {
public:
  [[nodiscard]] std::string getName() const override { return "NequIP Reader"; }
  [[nodiscard]] std::vector<std::string> getExtensions() const override {
    return {".nequip", ".xyz", ".extxyz"};
  }
  ...
  /**
   * @struct CommentData
   * @brief Intermediate storage for parsed NequIP Extended XYZ comment line metadata and column
   * layout.
   */
  struct CommentData {
...
private:
  ...
  static correlation::core::Cell parseNequipFrame(const char *data, size_t size);
};
